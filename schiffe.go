...
func (p *Player) init(usaddr net.Addr) {
	p.UserAddr = usaddr
...
func (g *Game) init(id int, idPlayer1 net.Addr, idPlayer2 net.Addr) {
	g.Id = id
...
func (g *Game) getPlayerId(player int) string {
	return g.Players[player].UserAddr
}
...
func (g *Game) getWinnerId() string {
	...
	return g.Players[g.WinningPlayer].UserAddr
}
...
func (g *Game) getLoserId() string {
	...
	return g.Players[loser].UserAddr
}
...
func leaveGame(id string) {
	if users[id].inGame != nil {
		...
		if users[id].player == 0 {
			players[users[id].inGame.Players[1].UserAddr].WriteJSON(Message{"notification", string(jsonb)})
		} else {
			players[users[id].inGame.Players[0].UserAddr].WriteJSON(Message{"notification", string(jsonb)})
		}
